...
const PerfilCompletoIrmao = ({ irmaoId, userData, onClose }) => {
  ...
  useEffect(() => {
    ...
    carregarDados();
  }, [irmaoId, anoPresencaSelecionado]);

  const carregarDados = async () => {
    try {
      setLoading(true);

      // 1. Carregar dados básicos do irmão
      ...
      // 2. Carregar anos disponíveis de presença
      const { data: sessoesAnosData } = await supabase
        .from('registros_presenca')
        .select('sessoes_presenca!inner(data_sessao)')
        .eq('membro_id', irmaoId);

      const anos = [...new Set(sessoesAnosData?.map(r => 
        new Date(r.sessoes_presenca.data_sessao).getFullYear()
      ))].sort((a, b) => b - a);
      
      setAnosDisponiveis(anos);

      // 3. Carregar presença (filtrado por ano)
      await carregarPresenca(irmaoData);

      // 4. Carregar financeiro
      await carregarFinanceiro();

      // 5. Carregar graus filosóficos
      await carregarGrausFilosoficos();

      // 6. Carregar comissões
  ...
  const carregarPresenca = async (irmaoData) => {
    try {
      // Buscar todas as sessões do ano ou todos os anos
      let querySessoes = supabase
        .from('sessoes_presenca')
        .select('id, data_sessao, grau_sessao_id');

      if (anoPresencaSelecionado !== 'todos') {
        querySessoes = querySessoes
          .gte('data_sessao', `${anoPresencaSelecionado}-01-01`)
          .lte('data_sessao', `${anoPresencaSelecionado}-12-31`);
      }

      const { data: sessoes, error: sessoesError } = await querySessoes
      ...
      sessoes?.forEach((sessao, index) => {
        ...
        if (grauSessao === 4) grauSessao = 1; // Administrativa = Aprendiz
        ...
        const dataInicio = irmaoData.data_ingresso_loja 
          ? new Date(irmaoData.data_ingresso_loja + 'T12:00:00')
          : irmaoData.data_iniciacao 
            ? new Date(irmaoData.data_iniciacao + 'T12:00:00') 
            : null;

        if (dataInicio && dataSessao < dataInicio) return;

        // Verificar situação bloqueadora
        ...
        if (situacaoBloqueadora) return; // Não computa
        ...
        // Guardar últimas 10 para visualização
        if (index < 10) {
          ultimasSessoes.push({
  ...
  const carregarFinanceiro = async () => {
    try {
      const { data: lancamentos } = await supabase
        ...
        .select('valor, status, categorias_financeiras(tipo)')
      ...
      lancamentos?.forEach(lanc => {
        if (lanc.categorias_financeiras?.tipo === 'receita') {
          receitas += parseFloat(lanc.valor || 0);
        } else if (lanc.categorias_financeiras?.tipo === 'despesa') {
          despesas += parseFloat(lanc.valor || 0);
        }
      });
  ...
  const carregarComissoes = async () => {
    try {
      ...
      const ativas = comissoes?.filter(c => !c.data_saida) || [];
      const inativas = comissoes?.filter(c => c.data_saida) || [];
  ...
  const carregarEventos = async () => {
    try {
      const { data: eventos } = await supabase
        ...
        .select('*, eventos_loja(nome, descricao, data_evento)')
        .eq('irmao_id', irmaoId)
        .order('eventos_loja(data_evento)', { ascending: false })
        .limit(20);
            ...
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="text-blue-100">
                  Grau: {obterGrauAtual()} | Iniciado em: {formatarData(irmao.data_iniciacao)}
                </p>
              </div>
            </div>
            <button 
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
          ...
          <section className="mb-8">
            <div className="flex justify-between items-center mb-4">
              ...
              <select 
                value={anoPresencaSelecionado}
                onChange={(e) => setAnoPresencaSelecionado(e.target.value)}
                      ...
                      {dadosPresenca.ultimasSessoes.map((sessao, idx) => (
                        <div 
                          ...
                          className={`w-12 h-12 rounded-lg flex items-center justify-center font-bold text-white text-lg ${
                            sessao.status === 'P' ? 'bg-green-500' :
                            sessao.status === 'J' ? 'bg-yellow-500' :
                            'bg-red-500'
                          }`}
                          title={`${formatarData(sessao.data)} - ${
                            sessao.status === 'P' ? 'Presente' :
                            sessao.status === 'J' ? 'Justificado' :
                            'Ausente'
                          }`}
                        >
                          {sessao.status}
            ...
            {dadosFinanceiro && (
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                  ...
                  <p className={`text-xl font-bold ${dadosFinanceiro.saldo >= 0 ? 'text-blue-800' : 'text-red-800'}`}>
                    {formatarMoeda(dadosFinanceiro.saldo)}
                  </p>
                </div>
                <div className={`p-4 rounded-lg border ${
                  ...
                  <p className={`text-sm font-medium mb-1 ${
                    dadosFinanceiro.situacao === 'Em dia' ? 'text-green-600' : 'text-red-600'
                  }`}>Situação</p>
                  <p className={`text-xl font-bold ${
                    dadosFinanceiro.situacao === 'Em dia' ? 'text-green-800' : 'text-red-800'
                  }`}>
                    {dadosFinanceiro.situacao === 'Em dia' ? '✅ Em dia' : '⚠️ Pendente'}
                  </p>
                </div>
              </div>
            )}
                ...
                {grausFilosoficos.map((grau, idx) => (
                  <div 
                    ...
                    className={`flex justify-between items-center p-4 ${
                      idx !== grausFilosoficos.length - 1 ? 'border-b border-gray-200' : ''
                    } hover:bg-gray-50`}
                  >
                    <div>
                      <p className="font-semibold text-gray-800">
                        Grau {grau.grau}° - {grau.nome_grau}
                      </p>
                      {grau.observacoes && (
                        <p className="text-sm text-gray-600 mt-1">{grau.observacoes}</p>
                      )}
                    </div>
          ...
          <section className="mb-8">
            ...
            <div className="mb-4">
              ...
              {comissoesAtivas.length > 0 ? (
                <div className="bg-white border border-green-200 rounded-lg overflow-hidden">
                  {comissoesAtivas.map((comissao, idx) => (
                    <div 
                      key={comissao.id}
                      className={`flex justify-between items-center p-3 ${
                        idx !== comissoesAtivas.length - 1 ? 'border-b border-green-100' : ''
                      } hover:bg-green-50`}
                    >
                      <span className="font-medium text-gray-800">{comissao.comissoes.nome}</span>
            ...
            <div>
              ...
              {comissoesInativas.length > 0 ? (
                <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                  {comissoesInativas.map((comissao, idx) => (
                    <div 
                      key={comissao.id}
                      className={`flex justify-between items-center p-3 ${
                        idx !== comissoesInativas.length - 1 ? 'border-b border-gray-100' : ''
                      } hover:bg-gray-50`}
                    >
                      <span className="text-gray-600">{comissao.comissoes.nome}</span>
                      <span className="text-xs text-gray-500">
                        Até {formatarData(comissao.data_saida)}
                      </span>
                    </div>
                  ))}
                </div>
          ...
          <section>
            ...
            {eventosParticipados.length > 0 ? (
              <div className="bg-white border border-gray-200 rounded-lg overflow-hidden max-h-80 overflow-y-auto">
                {eventosParticipados.map((evento, idx) => (
                  <div 
                    key={evento.id}
                    className={`p-4 ${
                      idx !== eventosParticipados.length - 1 ? 'border-b border-gray-200' : ''
                    } hover:bg-gray-50`}
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <p className="font-semibold text-gray-800">{evento.eventos_loja.nome}</p>
                        {evento.eventos_loja.descricao && (
                          <p className="text-sm text-gray-600 mt-1">{evento.eventos_loja.descricao}</p>
                        )}
                      </div>
                      <span className="text-sm text-gray-500 ml-4 whitespace-nowrap">
                        {formatarData(evento.eventos_loja.data_evento)}
                      </span>
                    </div>
                  </div>
                ))}
