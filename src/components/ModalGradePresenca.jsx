...
export default function ModalGradePresenca({ onFechar, periodoInicio, periodoFim }) {
  ...
  const carregarDados = async () => {
    try {
      ...
      if (e1) throw e1;
      console.log('✅ Sessões encontradas:', sessoesData?.length);
      ...
      if (e2) throw e2;
      console.log('✅ Irmãos encontrados:', irmaosData?.length);

      // 3. Buscar TODOS os registros (sem filtro de sessão para testar)
      const { data: registrosData, error: e3 } = await supabase
        .from('registros_presenca')
        .select('sessao_id, membro_id, presente, justificativa');

      if (e3) throw e3;
      console.log('✅ Registros TOTAIS no banco:', registrosData?.length);

      // Filtrar registros apenas das sessões do período
      const sessaoIds = sessoesData.map(s => s.id);
      const registrosFiltrados = registrosData.filter(r => sessaoIds.includes(r.sessao_id));
      console.log('✅ Registros do período:', registrosFiltrados?.length);
