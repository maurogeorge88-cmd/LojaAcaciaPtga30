...
const LVAZIO={categoria_id:'',descricao:'',valor:'',data_lancamento:HOJE.toISOString().slice(0,10),data_vencimento:'',forma_pagamento:'',cunhadas_selecionadas:[],eh_mensalidade:true};
...
export const FinanceiroCunhadas=({userData})=>{
  ...
  const cunhadaJaPagouLote=(cid)=>{
    if(!fLote.eh_mensalidade)return false;
          ...
          <div style={s.mol}>
            ...
            <div style={s.mb}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1rem',marginBottom:'1rem'}}>
                ...
                <Lbl l="Tipo de lançamento" st={{gridColumn:'1/-1'}} ch={
                  <div style={{display:'flex',gap:'0.6rem'}}>
                    <button type="button" onClick={()=>{
                      const{mes,ano}=(()=>{const[y,m]=fLote.data_lancamento.split('-');return{mes:parseInt(m,10),ano:parseInt(y,10)};})();
                      ...
                      setFLote({...fLote,eh_mensalidade:true,cunhadas_selecionadas:semPagas});
                    }} style={{flex:1,padding:'0.55rem 0.85rem',borderRadius:'var(--radius-lg)',cursor:'pointer',fontWeight:'700',fontSize:'0.82rem',border:`2px solid ${fLote.eh_mensalidade?'var(--color-accent)':'var(--color-border)'}`,background:fLote.eh_mensalidade?'var(--color-accent-bg)':'var(--color-surface-2)',color:fLote.eh_mensalidade?'var(--color-accent)':'var(--color-text-muted)'}}>📅 Mensalidade</button>
                    <button type="button" onClick={()=>setFLote({...fLote,eh_mensalidade:false})} style={{flex:1,padding:'0.55rem 0.85rem',borderRadius:'var(--radius-lg)',cursor:'pointer',fontWeight:'700',fontSize:'0.82rem',border:`2px solid ${!fLote.eh_mensalidade?'var(--color-accent)':'var(--color-border)'}`,background:!fLote.eh_mensalidade?'var(--color-accent-bg)':'var(--color-surface-2)',color:!fLote.eh_mensalidade?'var(--color-accent)':'var(--color-text-muted)'}}>🧾 Outro lançamento</button>
                  </div>
                }/>
                <div style={{gridColumn:'1/-1',display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'1rem'}}>
                  <Lbl l="Data lançamento" ch={<input type="date" style={s.inp} value={fLote.data_lancamento} onChange={e=>{
                    ...
                    if(!fLote.eh_mensalidade){setFLote({...fLote,data_lancamento:novaData});return;}
