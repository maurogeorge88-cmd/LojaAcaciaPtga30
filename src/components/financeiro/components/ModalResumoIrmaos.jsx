...
export default function ModalResumoIrmaos({ isOpen, onClose, resumoIrmaos }) {
  ...
  // Filtros aplicados apenas na tabela de ativos
  ...
  // Totais para os cards de resumo (apenas ativos)
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.75rem', marginBottom: '1.25rem' }}>
            <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)', padding: '0.85rem' }}>
              ...
              <p style={{ margin: 0, fontSize: '1.75rem', fontWeight: '800', color: '#2563eb' }}>{ativosExibir.length}</p>
            </div>
            <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)', padding: '0.85rem' }}>
          ...
          {/* ── Tabela 1: Irmãos Ativos / Licenciados ── */}
          {ativosExibir.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--color-text-muted)', marginBottom: '1.5rem' }}>
            ...
            <TabelaIrmaos
              ...
              corTitulo="#2563eb"
              bgTitulo="rgba(37,99,235,0.06)"
            />
          )}

          {/* ── Tabela 2: Irmãos Inativos com Pendências ── */}
          {inativos.length > 0 && (
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', margin: '0.5rem 0 0.25rem' }}>
                <div style={{ flex: 1, height: '1px', background: 'var(--color-border)' }} />
                <span style={{ fontSize: '0.72rem', fontWeight: '700', color: '#ea580c', textTransform: 'uppercase', letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>
                  ⚠️ Pendências de Irmãos Inativos
                </span>
                <div style={{ flex: 1, height: '1px', background: 'var(--color-border)' }} />
              </div>
              <TabelaIrmaos
